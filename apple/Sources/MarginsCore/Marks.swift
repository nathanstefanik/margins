...
public enum Marks {
    ...
    /// Generates a fresh mark id: a time-ordered 10-character Crockford
    /// base32 id (`CoreID`), so marks sort by creation time in files.
    /// Callers retry on the (unlikely) collision with an existing id.
